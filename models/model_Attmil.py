...
##############################################################################
def to_fixed_size_bag(bag: torch.Tensor, bag_size: int = 512, wsiName: str = "name") -> Tuple[torch.Tensor, int, list]:
    # get up to bag_size elements
    bag_idxs = torch.randperm(bag.shape[0])[:bag_size]
    bag_idxsSaved = bag_idxs.tolist()
    bag_samples = bag[bag_idxs]

    # zero-pad if we don't have enough samples
    zero_padded = torch.cat((bag_samples,
                              torch.zeros(bag_size-bag_samples.shape[0], bag_samples.shape[1])))
    return zero_padded, min(bag_size, len(bag)), bag_idxsSaved
...
class MILBagTransform(Transform):
    ...
    def _draw(self, f: os.PathLike) -> Tuple[torch.Tensor, int, list, str]:
        name = os.path.basename(str(f))
        featureFile = torch.load(f)
        zero_paddedFeature, trueBagSize, bag_idxsSaved = to_fixed_size_bag(featureFile, bag_size=self.max_bag_size, wsiName = name)
        return zero_paddedFeature, trueBagSize, bag_idxsSaved, name
