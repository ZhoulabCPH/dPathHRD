...
def ConcatCohorts_Classic(imagesPath, cliniTablePath, slideTablePath,  label, reportFile, outputPath, csvName, patientNumber = 'ALL'):
    ...
    for imgCounter in range(len(imagesPath)):
        ...
        for path in currentSlideTable['FILENAME']:
            FILEPATH.append(Path(currentImgPath, path))
...
def GetTiles(csvFile, label, maxBlockNum, target_labelDict, test = False, seed = 23, filterPatients = []):
    ...
    for index, patientID in enumerate(tqdm(patientsUnique)):
        ...
        for item in range(len(selectedData)):
            tempTiles.extend([os.path.join(selectedData['FILEPATH'][item], i) for i in os.listdir(selectedData['FILEPATH'][item])])
