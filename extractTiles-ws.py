...
NUM_THREADS = 8
DEFAULT_JPG_MPP = 0.2494        # 这个0.5表示默认是40x
    ...
    def build_generator(self, size_px, size_um, stride_div, case_name, tiles_path,  category, fileSize, export = False, augment = False):
        ...
        def generator():
            for ci in range(len(self.coord)):
                ...
                if median_brightness > 660:     # 这边用来过滤patches，如果平均亮度大于660则筛去
                    continue
                ...
                edge  = cv2.Canny(region, 40, 100)  # opencv的canny边缘检测算法，
                ...
                if exportFlag:                 
                    imageio.imwrite(join(tiles_path, case_name +'_('+str(c[0])+','+str(c[1])+').jpg'), region)  # 符合标准则真正地保存patch
                    if augment:
...
class Convoluter:
    ...
    def convolute_slides(self):
        ...
        '''
        ignoredFile_list = []
        if not os.path.exists(join(self.SAVE_FOLDER, "WSI_Patches")):
            os.makedirs(join(self.SAVE_FOLDER, "WSI_Patches"))
    ...
    def export_tiles(self, slide, pb, ignoredFile_list):
        ...
        tiles_path = whole_slide.export_folder + '/' + "WSI_Patches"
...
def get_args():
    ...
    parser.add_argument(
        '-s', '--slide', default='', help='Path to whole-slide image (SVS or JPG format) or folder of images (SVS or JPG) to analyze.')
    ...
    parser.add_argument('--um', type=float, default=256,
...
if __name__ == ('__main__'):
    ...
    if isfile(args.slide):
        ...
        if os.path.exists(join(args.out, "WSI_Patches")):
            temp = os.listdir(os.path.join(args.out, 'WSI_Patches'))
            for item in temp:
                for s in slide_list:
