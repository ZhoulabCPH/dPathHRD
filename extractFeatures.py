...
from models.extractors.ctran import ctranspath
...
def ExtractFeatures(data_dir, feat_dir, batch_size, target_patch_size=-1, filterData=True):
    ...
    td = torch.load(r'./models/extractors/ctranspath.pth')
    ...
    for bag_candidate_idx in range(total):
        ...
        if not os.path.exists(os.path.join(feat_dir, bag_base + '.pt')):
            ...
            output_file_path = Compute_w_loader(file_path, output_path, model=model, batch_size=batch_size, verbose=1, print_every=20, target_patch_size=target_patch_size)
